...
# --- TELA DE LOGIN / CADASTRO CENTRALIZADA EM CARD ÚNICO ---
...
if st.session_state.menu_atual == "🏠 Home":
    ...
    st.subheader("📷 Consulta Rápida por Seleção de Pallet")
    st.info(
        "Como a leitura automática de QR Code por câmera em servidores web exige configurações avançadas de hardware que costumam falhar na nuvem, selecione o corredor e o pallet abaixo para visualizar os vinhos instantaneamente:"
    )

    c_corr_sel = st.selectbox("Selecione o Corredor:", LISTA_CORREDORES)
    c_pall_sel = st.selectbox("Selecione o Pallet:", LISTA_PALLETS)
    pallet_alvo = f"{c_corr_sel} - {c_pall_sel}"

    if st.button("Consultar Itens deste Pallet", use_container_width=True):
        vinhos_pallet = [
            v for v in st.session_state.estoque if v.get("pallet") == pallet_alvo
        ]
        if vinhos_pallet:
            st.success(
                f"Encontrados {len(vinhos_pallet)} vinhos no {pallet_alvo}:"
            )
            for v in vinhos_pallet:
                st.markdown(
                    ...
                    <div class="wine-card">
                        <div class="wine-title">🍷 {v.get('nome')} ({v.get('safra')})</div>
                        <p class="wine-text"><b>Tipo:</b> {v.get('tipo')} | <b>Embalagem:</b> {v.get('caixa')} | <b>Lado:</b> {v.get('lado')}</p>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            st.warning(
                f"⚠️ Nenhum vinho cadastrado no {pallet_alvo} no momento."
            )
