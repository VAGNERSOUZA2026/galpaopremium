...
if st.session_state.menu_atual == "🏠 Home":
    ...
    for l in logs[:50]:
        st.markdown(f"- **{l['data_hora']}** | *{l['usuario']}* - {l['acao']}: {l['detalhes']}")import json
import os
import shutil
from datetime import datetime, timezone, timedelta
import pandas as pd
...
if st.session_state.menu_atual == "🏠 Home":
    ...
    st.subheader("⚙️ Painel de Desenvolvedor / Ferramentas de Limpeza")
    if st.button("🗑️ Resetar / Limpar Todos os Pedidos da Matriz"):
        if os.path.exists(ARQUIVO_PEDIDOS):
            os.remove(ARQUIVO_PEDIDOS)
        st.session_state.pedidos = []
