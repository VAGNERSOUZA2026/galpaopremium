...
if st.session_state.menu_atual == "🏠 Home":
    ...
    st.subheader("🏢 Painel da Matriz - Cadastro e Gestão de Pedidos")
    
    with st.form("form_cadastrar_pedido_matriz"):
        id_pedido_novo = st.text_input("Código de Barras do Mapa (Ex: 1234552)").strip()
        arquivo_pedido = st.file_uploader("Arquivo de Pedido (Excel ou TXT)", type=["xlsx", "xls", "txt"])
        texto_pedido_rapido = st.text_area("Ou digite um item por linha no formato rápido (Nome / Safra / Qtd):")
        
        btn_salvar_pedido = st.form_submit_button("💾 Salvar Pedido no Sistema")
        
        if btn_salvar_pedido:
            if not id_pedido_novo:
                st.error("Informe o código identificador/número do pedido.")
            else:
                itens_novos = []
                if texto_pedido_rapido.strip():
                    linhas = texto_pedido_rapido.strip().split("\n")
                    for linha in linhas:
                        partes = [p.strip() for p in linha.split("/")]
                        nome_i = partes[0] if len(partes) > 0 else "Vinho Diversos"
                        safra_i = partes[1] if len(partes) > 1 else ""
                        try:
                            qtd_i = int(partes[2]) if len(partes) > 2 else 1
                        except:
                            qtd_i = 1
                        itens_novos.append({"nome": nome_i, "safra": safra_i, "quantidade": qtd_i, "qtd_separada": 0, "separado": False})
                elif arquivo_pedido is not None:
                    try:
                        if arquivo_pedido.name.endswith('.xlsx') or arquivo_pedido.name.endswith('.xls'):
                            df_p = pd.read_excel(arquivo_pedido)
                        else:
                            df_p = pd.read_csv(arquivo_pedido, sep=None, engine='python')
                        for _, row in df_p.iterrows():
                            nome_i = str(row.iloc[0]) if len(row) > 0 else "Vinho"
                            safra_i = str(row.iloc[1]) if len(row) > 1 else ""
                            try:
                                qtd_i = int(row.iloc[2]) if len(row) > 2 else 1
                            except:
                                qtd_i = 1
                            itens_novos.append({"nome": nome_i, "safra": safra_i, "quantidade": qtd_i, "qtd_separada": 0, "separado": False})
                ...
                if not itens_novos:
                    itens_novos.append({"nome": "Vinho Padrão Matriz", "safra": "2023", "quantidade": 1, "qtd_separada": 0, "separado": False})
                ...
                st.session_state.pedidos.append(novo_pedido_obj)
                salvar_pedidos(st.session_state.pedidos)
                sincronizar_estoque_com_pedidos(st.session_state.pedidos, st.session_state.estoque)
                registrar_log(st.session_state.usuario_logado['nome'], "Cadastrou Pedido Matriz", f"Pedido {id_pedido_novo}")
                st.success(f"Pedido {id_pedido_novo} cadastrado com sucesso!")
                st.rerun()

    st.markdown("---")
    st.subheader("📋 Pedidos Cadastrados Atualmente:")
    if not st.session_state.pedidos:
        st.info("Nenhum pedido registrado.")
    else:
        for p in st.session_state.pedidos:
            st.markdown(f"**Pedido ID:** {p['id']} | **Data:** {p.get('data', 'N/A')} | **Status:** {p.get('status', 'Pendente')}")
