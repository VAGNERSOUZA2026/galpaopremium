...
if st.session_state.menu_atual == "🏠 Home":
    ...
    _, col_logo, _ = st.columns([2, 1.5, 2])
    with col_logo:
        # Exibe a imagem da logo centralizada
        if os.path.exists("logo_vinho.png"):
            st.image("logo_vinho.png", width=160)
        else:
            arquivos_pasta = [f for f in os.listdir('.') if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
            if arquivos_pasta:
                st.image(arquivos_pasta[0], width=160)
        ...
        # Bloco unificado em HTML para evitar quebras de linha indesejadas
        st.markdown(
            f"""
            <div style="text-align: center; margin-top: 10px; margin-bottom: 25px;">
                <p style="color: #666; font-size: 1rem; margin: 0;">{saudacao},</p>
                <h2 style="color: #7A1C2E; font-size: 2rem; margin: 5px 0 0 0; font-weight: 700;">{nome_usuario}! 👋</h2>
            </div>
            """, 
            unsafe_allow_html=True
        )
